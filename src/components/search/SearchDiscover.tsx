          ...
          <div className="responsive-single-column-narrow grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3.5">
            {(hasSearch ? searchResults : discoveryRecipes).map((r) => (
              <Link key={r.slug} to={r.authorUsername ? `/${r.authorUsername}/${r.slug}` : `/recipe/${r.slug}`}
                ...
                {r.originalLanguage && (
                  <span className="absolute left-3 top-3 rounded-full bg-white/90 px-2 py-1 text-stone-900 shadow-sm backdrop-blur-md"
            ...
            <div className="space-y-2">
              ...
              <select value={recipeLanguageFilter} onChange={(event) => setRecipeLanguageFilter(event.target.value)}
                ...
                {recipeLanguageOptions.map((code) => (
                  <option key={code} value={code}>{t(`detail.language.${code}`)}</option>
                ))}
              </select>
            </div>
